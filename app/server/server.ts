...
type AppConfig = {
  ...
  data?: {
    ...
    tables: {
      ...
      openQueue: string;
      // Optional — the ML disposition-recommendations table. The TRAINEE builds
...
const tablesSchema = z.object({
  paymentPosition: z.string().min(1),
  openQueue: z.string().min(1),
  // Optional — the ML disposition-recommendations table. The trainee builds it
  // (Build 2 ML step); empty/omitted until then. db/sync.ts tolerates it.
  dispositionRecommendations: z.string().optional(),
});
