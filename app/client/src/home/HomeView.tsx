...
import { fetchActivity } from '@/lib/payments';
...
const HERO = {
  name: 'Della Okonkwo',
  role: 'Deputy Commissioner for Program Integrity · Sentinel Benefits',
};

const STORY = {
  headline: 'Fraud spike threatens $361K in improper-payment exposure.',
  situation:
    'A cross-agency fraud-match feed + eligibility-data refresh landed ~3 weeks ago and surfaced a SPIKE of high-risk pre-disbursement payments. 397 flagged payments across TANF, SNAP, Child Care, Disability, Veteran programs. High-risk cases dominate the exposure. Della needs to triage fast and dispatch case workers.',
  goal: 'Find the highest-risk flagged payments, get the recommended dispositions, approve case actions.',
};
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-medium text-foreground">
            {dispositionLabel} · Payment {event.payment_id}
          </div>
          <div className="text-xs text-muted-foreground mt-1">
            {event.status} by {event.by} · {timeStr}
            {event.predicted_recovery_usd && (
              <> · predicted recovery +${event.predicted_recovery_usd.toLocaleString()}</>
            )}
          </div>
        </div>
      </div>
