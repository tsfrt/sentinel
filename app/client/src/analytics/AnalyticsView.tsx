      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-10 space-y-6 sm:space-y-10">
        <div>
          ...
          <h1 className="display text-4xl font-semibold tracking-tight text-foreground mb-2">
            Where the improper-payment risk concentrates.
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <ChartCard
            ...
            scope="Last ~9 weeks — the feed lands ~3 weeks ago"
          ...
          <ChartCard
            title="Flagged payments by program"
            scope="Current queue"
        ...
        <ChartCard title="Highest-exposure flagged payments" scope="By improper-payment exposure" flush>
...
type WorstShortfallRow = {
  ...
  n_signals: number;
  signal_list: string;
  improper_payment_exposure_usd: number;
      ...
      <table className="w-full text-sm">
        <thead className="bg-muted text-xs uppercase tracking-wider text-muted-foreground border-b border-border">
          <tr>
            ...
            <th className="text-left px-4 py-2 font-semibold"># signals</th>
            <th className="text-right px-4 py-2 font-semibold">Recovery $</th>
        ...
        <tbody className="divide-y divide-border">
          {rows.map((row, i) => (
            <tr key={i} className="hover:bg-muted/40 transition-colors">
              <td className="px-4 py-2 font-medium">{row.payment_id}</td>
              <td className="px-4 py-2 text-muted-foreground">{row.program}</td>
              <td className="px-4 py-2 max-w-[280px] truncate" title={row.signal_list}>
                {row.signal_list}
              </td>
              <td className="px-4 py-2 font-mono">{row.n_signals}</td>
              <td className="px-4 py-2 text-right font-mono text-muted-foreground">
                ${row.projected_recovery_usd.toLocaleString(undefined, {
                  maximumFractionDigits: 0,
                })}
              </td>
              <td className="px-4 py-2 text-right font-mono">
                ${row.improper_payment_exposure_usd.toLocaleString(undefined, {
                  maximumFractionDigits: 0,
                })}
              </td>
            </tr>
